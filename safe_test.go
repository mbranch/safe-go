...
func TestGroupTryGo(t *testing.T) {
	...
	g.TryGo(func() error { panic("internal error") })
	err := g.Wait()
	if err == nil {
		t.Fatal("expected error")
	}
...
func TestGroupSetLimit(t *testing.T) {
	var g safe.Group
	g.SetLimit(0) // Expect no goroutines to run.
	g.TryGo(func() error { return errors.New("internal error") })
	err := g.Wait()
	assert.Nil(t, err) // No error returned if no goroutines run.
}
